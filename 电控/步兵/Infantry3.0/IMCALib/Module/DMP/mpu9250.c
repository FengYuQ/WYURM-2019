...
unsigned char MPU9250_Init(void)
{
	...
	status=HAL_I2C_IsDeviceReady(&hi2c1,MPU9250_ADDR, 10, HAL_MAX_DELAY);//I2C准备函数
  unsigned char res=0;
	pdata=0x80; //复位MPU
...
//   MPU_Write_Byte(MPU9250_ADDR,MPU_PWR_MGMT1_REG,0X80);//复位MPU9250
	...
	//唤醒MPU
	pdata=0x01; // 7位 1 重启  6位睡眠模式1 睡眠 2 唤醒   3位 传感器0 开启    0-2位 时钟选择  01 PLL使用XZ轴陀螺作为参数
	HAL_I2C_Mem_Write(&hi2c1,MPU9250_ADDR, MPU_PWR_MGMT1_REG, 1, &pdata, 1, HAL_MAX_DELAY); //写0x80复位
...
//        MPU_Write_Byte(MPU9250_A/‘DDR,MPU_PWR_MGMT1_REG,0X01);  	//设置CLKSEL,PLL X轴为参考
			pdata=0x00;
